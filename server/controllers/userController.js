...
export const register = async (req, res) => {
    try {
        ...
        return res.json({success: true, user: {email: user.email, name: user.name}, message: "Successfully Signup"});
    } catch (error) {
...
export const login = async (req, res) => {
    try {
        ...
        res.json({success: true, user: {email: user.email, name: user.name}, message: "Successfully login", isAdmin});
    } catch (error) {
        res.json({success: false, message: error.message});
...
export const isAuth = async (req, res) => {
    try {
        const user = req.user;
        const isAdmin = (user.email === process.env.ADMIN_EMAIL) ? true : false;
        return res.json({ success: true, user, isAdmin });
    } catch (error) {
        res.json({success: false, message: error.message});
